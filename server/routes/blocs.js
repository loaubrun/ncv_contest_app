...
router.post("/getBlocs", (req, res) => {
    ...
    db.query("SELECT bloc.blocId, bloc.name, bloc.photoName, color, juged, bloc.secteur,  COALESCE(COUNT(perf_bloc.blocId), 0) AS checked FROM bloc LEFT JOIN perf_bloc ON bloc.blocId = perf_bloc.blocId AND perf_bloc.climberId = ? GROUP BY bloc.blocId", 
        [climberId],
        (err, results) => {
        if (err) return res.status(500).json({ error: "Erreur serveur" });
        res.json(results);
...
router.post("/addBloc", upload.single("image"), (req, res) => {
    const { name, color, juged, secteur } = req.body;
    const photoName = req.file ? `/uploads/${req.file.filename}` : null;

    if (!name || !color || !photoName || !juged || !secteur) {
        return res.status(400).json({ error: "Données incomplètes" });
    }

    db.query("INSERT INTO Bloc (name, color, photoName, juged, secteur) VALUES (?, ?, ?, ?, ?)", 
        [name, color, photoName, juged, secteur], 
        (err, result) => {
            if (err) return res.status(500).json({ error: "Erreur serveur" });
...
router.post("/updateBloc", upload.single("image"), (req, res) => {
    const { id, name, color, juged, secteur } = req.body;
    const photoName = req.file ? `/uploads/${req.file.filename}` : null;

    const sql = photoName 
        ? "UPDATE Bloc SET name = ?, color = ?, juged = ?, secteur = ?, photoName = ? WHERE blocId = ?"
        : "UPDATE Bloc SET name = ?, color = ?, juged = ?, secteur = ? WHERE blocId = ?";

    const params = photoName 
        ? [name, color, juged, secteur, photoName, id] 
        : [name, color, juged, secteur, id];
